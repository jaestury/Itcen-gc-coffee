...
export function Summary({itmes = []}) {
    const totalPrice = itmes.reduce((prev, curr) => prev + (curr.price * curr.count), 0);
    return (
        <React.Fragment>
            <div>
            ...
            <hr/>
            {itmes.map(v => <SummaryItem key={v.id} count={v.count} ProductName={v.productName}/>)}
            <form>
                <div className="mb-3">
